    ...
    try {
      ...
      for (const id_empresa of empresas) {
        ...
        for (const c of citas) {
          ...
          if (c.cliente_correo) await enviarCorreo(c.cliente_correo, "Recordatorio: visita de hoy", html);
          if (correoEmpresa) await enviarCorreo(correoEmpresa, "Recordatorio: visita de hoy (cliente)", html);
    ...
    try {
      ...
      for (const id_empresa of empresas) {
        ...
        for (const c of citas) {
          ...
          const citaDate = new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate(), h, min, 0, 0);
          const diffMin = Math.round((citaDate.getTime() - ahora.getTime()) / 60000);

          if (diffMin <= 65 && diffMin >= 55) { // ventana de ~1h antes
            ...
            `;
            const correoEmpresa = process.env.EMPRESA_NOTIF || null;
            if (c.cliente_correo) await enviarCorreo(c.cliente_correo, "Recordatorio: tu visita es en 1 hora", html);
            if (correoEmpresa) await enviarCorreo(correoEmpresa, "Recordatorio: visita de cliente en 1 hora", html);
            await AgendaModel.registrarRecordatorio(c.id, tipo);
          }
        }
      }
